...
const getApprovedProgram = async (userId) => {
  const query = `
    SELECT program_type
    FROM applications
...
exports.getAttendanceRecords = async (userId) => {
  ...
  const approvedProgram = await getApprovedProgram(userId);
  if (!approvedProgram) {
    const error = new Error('Attendance is available only after application approval.');
...
exports.getTodayAttendance = async (userId) => {
  ...
  return {
    ...
    programType: approvedProgram.program_type
  };
};
...
exports.timeIn = async (userId) => {
  ...
  if (existingRows.length === 0) {
    ...
    await db.execute(insertQuery, [userId, approvedProgram.program_type]);
  } else {
    const updateQuery = `
    ...
    await db.execute(updateQuery, [approvedProgram.program_type, existingRows[0].attendance_id]);
  }
