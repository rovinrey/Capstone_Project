...
const BeneficiaryAttendance = () => {
    ...
    const fetchRecords = async (beneficiaryId: number) => {
        ...
        setError(null);
        try {
            const res = await axios.get(`http://localhost:5000/api/attendance/beneficiary/${beneficiaryId}`);
            setRecords(res.data);
        } catch (err: any) {
            console.error(err);
    ...
    const addRecord = async () => {
        if (!id) return;
        try {
            await axios.post(`http://localhost:5000/api/attendance/beneficiary/${id}`, {
                date: newDate,
                status: newStatus,
                notes: newNotes || null,
            });
